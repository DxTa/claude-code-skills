---
description: Generate comprehensive AGENTS.md for any repository
---

# Repository Analysis & AGENTS.md Generation

Analyze the repository and generate a comprehensive `AGENTS.md` development guide for AI assistants and developers.
...
7. **Repository instruction discovery** (`AGENTS.md` / `CLAUDE.md`)

---

## Phase 0: Instruction File Discovery

**MANDATORY FIRST STEP** - Discover and apply repository instruction files before analysis.
...
- Apply nearest-file precedence for that subtree.
- If both exist at the same level, apply both and prioritize `AGENTS.md` if directives conflict.

### 0.3 Track Instruction Coverage
- Record which instruction files were found and where they apply.
- Note any conflicts and how precedence resolved them.
...
- `README.md`, `CONTRIBUTING.md`, `docs/`, existing `AGENTS.md`, existing `CLAUDE.md`, `.github/`
- Include both root-level and scoped instruction files discovered in Phase 0
...
- Instruction-file precedence rules (`AGENTS.md` / `CLAUDE.md`)

Avoid duplicating large code blocks - reference files instead.
...
- Read root-level instruction files first
- For work in a subfolder, use the nearest `AGENTS.md` / `CLAUDE.md` found in that path's ancestor chain
- If both files exist at the same directory depth, apply both and prioritize `AGENTS.md` on conflicts

**When to Re-evaluate Scope:**
- Switching to a different directory tree
- Reviewing changes in files with different nearest instruction files
- Handing tasks to subagents that operate in different subfolders
...
- [ ] **CRITICAL: Under 5,000 tokens** (keep concise guidance only)
...
2. **Execute Phase 0 first** (instruction file discovery)
...
**Now execute Phase 0 (instruction discovery), then analysis and generation.**
