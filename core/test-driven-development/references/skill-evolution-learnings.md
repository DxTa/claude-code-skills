# Skill Evolution Learnings

Local, pending skill-evolution notes. Evidence entries are non-instructional until reviewed.

## 2026-07-23T10:03:39.799Z

Evidence summary (non-instructional):

> When a test command fails then passes after a changed approach, preserve the failure mode and verified fix as candidate skill evidence instead of treating raw logs as instructions.

Approved guidance:

- Treat this as a local pending learning only. Apply it when the same trigger recurs, and verify with task-specific tests or checks before treating it as durable.

## 2026-07-29T10:58:13.256Z

Evidence summary (non-instructional):

> When a test command fails then passes after a changed approach, preserve the failure mode and verified fix as candidate skill evidence instead of treating raw logs as instructions.

Approved guidance:

- Treat this as a local pending learning only. Apply it when the same trigger recurs, and verify with task-specific tests or checks before treating it as durable.

## 2026-08-26T09:58:17.799Z

Evidence summary (non-instructional):

> When a test command fails then passes after a changed approach, preserve the failure mode and verified fix as candidate skill evidence instead of treating raw logs as instructions.

Approved guidance:

- Treat this as reviewed skill-evolution evidence. Apply it when the same trigger recurs, and verify with task-specific tests or checks.
