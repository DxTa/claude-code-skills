...
const DANGEROUS_XPATH_PATTERNS = [
  /javascript:/i,
  /<script/i,
  /onerror\s*=/i,
  /onload\s*=/i,
...
function validateXPath(xpath) {
  ...
  for (const pattern of DANGEROUS_XPATH_PATTERNS) {
    const match = xpath.match(pattern);
    if (match) {
      throw new Error(`XPath injection detected: ${match[0]}`);
    }
  }
